    ...
    <div v-else-if="detail" class="detail-content">
        ...
        <a-descriptions :column="2">
          ...
          <a-descriptions-item label="商品图片" :span="2">
             <a-image-preview-group>
                <a-space>
                  <a-image
                    v-for="(img, index) in (detail.image ? detail.image.split(',') : [])"
                    :key="index"
                    :width="100"
                    :src="img"
                  />
      ...
      <a-card title="基础属性" :bordered="false" class="section-card">
        <a-descriptions :column="3" size="small" bordered>
          <a-descriptions-item 
            v-for="(value, key) in detail.baseAttrs" 
            :key="key" 
            :label="key"
          >
            {{ value }}
...
import { getSpuDetail, updateSpuStatus } from '@cloudfarming/core/api/spu';
import { getCategoryById } from '@cloudfarming/core/api/category';
import type { SpuDetailRespDTO } from '@cloudfarming/core/api/spu';

const route = useRoute();
const router = useRouter();
const loading = ref(false);
const detail = ref<SpuDetailRespDTO | null>(null);
