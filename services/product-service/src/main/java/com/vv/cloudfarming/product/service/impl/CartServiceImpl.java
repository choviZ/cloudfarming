...
import com.vv.cloudfarming.product.dao.mapper.CartArchiveMapper;
import com.vv.cloudfarming.product.dto.domain.CartItemDTO;
...
public class CartServiceImpl implements CartService {
    ...
    public Boolean addToCart(CartItemAddReqDTO requestParam) {
        ...
        try {
            ...
            if (cartItemJson == null) {
                ...
                if (cartArchive != null) {
                    ...
                    for (String cartItemStr : cartItemStrings) {
                        CartItemDTO item = JSON.parseObject(cartItemStr, CartItemDTO.class);
            ...
            if (cartItemJson != null) {
                // 商品已在购物车中，更新数量
                cartItem = JSON.parseObject(cartItemJson, CartItemDTO.class);
    ...
    public Boolean updateCartItem(CartItemUpdateReqDTO requestParam) {
        try {
            ...
            // 更新购物车项
            CartItemDTO cartItem = JSON.parseObject(cartItemJson, CartItemDTO.class);
    ...
    public CartRespDTO getCart() {
        try {
            ...
            // 转换为CartItemDTO列表
            List<CartItemDTO> cartItems = cartItemsMap.values().stream()
                    .map(obj -> JSON.parseObject(obj.toString(), CartItemDTO.class))
    ...
    public Boolean selectAllCartItems(Boolean selected) {
        try {
            ...
            for (Map.Entry<Object, Object> entry : cartItemsMap.entrySet()) {
                ...
                CartItemDTO cartItem = JSON.parseObject(entry.getValue().toString(), CartItemDTO.class);
                cartItem.setSelected(selected);
